...
export const loginStudent = async (req, res) => {
  try {
    ...
    // Find the student by roll number (email remains as is)
...
// Login user for roles other than Student (Faculty, Admin, DEO)
export const loginUser = async (req, res) => {
  try {
    const { email, password } = req.body;
    ...
    if (user.role === "Faculty" || user.role === "Admin" || user.role === "DEO") {
