...
// Reset password for Faculty and other roles
export const resetPasswordOtherRoles = async (req, res) => {
  try {
    const { email, newPassword, confirmPassword } = req.body;
    if (!email || typeof email !== "string" || !email.includes("@")) {
...
export const loginUser = async (req, res) => {
  try {
    ...
    const user = isStudent
      ? await User.findOne({ email: emailLowerCase })
      : await User.findOne({ email: emailLowerCase });
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
...
export const getProfileImage = async (req, res) => {
  try {
    ...
    if (!user || !user.profilePicture) {
      return res.status(404).sendFile(path.join(__dirname, '..', 'uploads', 'default-avatar.png'));
    }
